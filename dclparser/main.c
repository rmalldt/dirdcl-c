...
int processDeclaration(char *declaration, char expectParameter)
{
    ...
    // Since the while loop gets an extra unneeded token, push it back in reverse order
    ...
    do
    {
        ...
        if (dcl(name, out, expectParameter) == ERROR) // dcl updates out based on input
            return ERROR;
        else if (tokentype != ';' && tokentype != ',' && tokentype != ')' && tokentype != '\n') // if returned output is not one of these chars
            return error("Syntax error");                                                       // an error occurred
        else
        {
...
int dirdcl(char *name, char *out, char expectParameter)
{
    ...
    {
        snprintf(name, MAXTOKEN, "%s", " unnamed"); // space added is a flag indicates that this is an
        expectParameter = FALSE;                    // unnamed parameter instead of a named parameter called unnamed
        for (int i = strlen(token) - 1; i >= 0; i--)
            unget_ch(token[i]); // push unused token back to input in reverse order
    }
    else
...
int gettoken(void)
{
    ...
    while ((c = get_ch()) == ' ' || c == '\t') // skip spaces and tabs
        ;
    if (c == '(')
    {
    ...
    else if (isalpha(c) || c == '_') // get name
    {
        for (*p++ = c; isalnum(c = get_ch()) || c == '_'; )
            *p++ = c;
        *p = '\0';
        unget_ch(c); // push back the unneeded extra char
...
// Concatenates str to end of dst; requires null terminated strings and dst buffer size.
// Returns null if provided bad pointers or buffer is too small, otherwise returns pointer to dst.
char *saferstrcat(char *dst, const char *str, size_t dstsize)
{
